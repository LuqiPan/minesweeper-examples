...
class PythonSolver:
  ...
  def my_solver(self):
    ...
    while True:
      ...
      for i in range(len(prob_board)):
        for j in range(len(prob_board[0])):
          if self.board[i+1][j] != None or self.board[i-1][j] != None or self.board[i][j+1] != None or self.board[i][j-1] != None or self.board[i-1][j-1] != None or self.board[i-1][j+1] != None or self.board[i+1][j-1] != Non or self.board[i+1][j-1] != None:
